...
app.get('/Productos/:FechaSync', async (req, res) => {
  ...
  const query = `Select P.CodigoProducto, P.Nombre, CONCAT(CAST((P.CodigoProducto) AS CHAR),' ',P.Nombre) as NombreBusqueda, P.IVA, P.CantidadxBulto, P.Existencia, IFNULL(P.Existencia02,0) as Existencia02, IFNULL(P.Existencia03,0) as Existencia03, (IFNULL(P.Existencia02,0) + IFNULL(P.Existencia03,0)) as ExistenciaVenta , (case when P.PedidoVenta is null then 0 else P.PedidoVenta end) as PedidoVenta, IFNULL((SELECT PP.PrecioMoneda FROM ProductosPrecios PP WHERE PP.CodigoProducto=P.CodigoProducto AND PP.Tarifa='A'),0) as PrecioMoneda, IFNULL((SELECT PP.PrecioMoneda FROM ProductosPrecios PP WHERE PP.CodigoProducto=P.CodigoProducto AND PP.Tarifa='B'),0) as PrecioMonedaB, P.CodigoGrupo, P.Marca, P.FechaSync from Productos P where Visible=1 and FechaSync >= ? Order by P.CodigoGrupo, P.Marca, P.Nombre`;
...
app.post('/CrearPedido2', async (req, res) => {
  try {
    ...
    const [contadorResult] = await pool.query(selectContadorQuery);
    const nuevoContador = contadorResult[0].Contador;
    const tar = pedidoCompleto.renglones[0].tarifa;


    // Lógica para insertar el pedido en Pedidos
    const insertQueryPedido = `INSERT INTO Pedidos (Numero, FechaEmision, FechaEntrega, CodigoCliente, TotalBruto, Descuento, Impuesto, Cargo, TotalPedido, PorcentajeDescuento, Vendedor, Comentarios, Tarifa, Almacen, Peso, Estatus, Usuario, Cambio, Moneda, TotalBruto2, Descuento2, Impuesto2, Cargo2, TotalPedido2, Idmoneda, ubicacion, EsEspera) VALUES (?, DATE_SUB(NOW(), INTERVAL 4 HOUR), DATE_SUB(NOW(), INTERVAL 4 HOUR), ?, round(?*(select cambio from MonedasCambio where idmoneda=2 order by fecha desc LIMIT 1),2), 0, round(?*(select cambio from MonedasCambio where idmoneda=2 order by fecha desc LIMIT 1),2), 0, round(?*(select cambio from MonedasCambio where idmoneda=2 order by fecha desc LIMIT 1),2)+round(?*(select cambio from MonedasCambio where idmoneda=2 order by fecha desc LIMIT 1),2), 0, ?, CONCAT('APP: ', ?), ?, '02', 0, 'PE', 'APP', (select cambio from MonedasCambio where idmoneda=2 order by fecha desc LIMIT 1), 'BsS', ?, 0, ?, 0, ?, 2, POINT(?, ?), ?)`;
    const valuesPedido = [nuevoContador, pedidoCompleto.codigoCliente, pedidoCompleto.totalNeto, pedidoCompleto.totalImpuesto, pedidoCompleto.totalNeto, pedidoCompleto.totalImpuesto, pedidoCompleto.vendedor, pedidoCompleto.comentario, tar, pedidoCompleto.totalNeto, pedidoCompleto.totalImpuesto, pedidoCompleto.totalPedido, pedidoCompleto.latitud, pedidoCompleto.longitud, pedidoCompleto.esEspera];
    const [pedidoResult] = await pool.query(insertQueryPedido, valuesPedido);
...
app.post('/Usuarios', async (req, res) => {
  ...
  const insertQuery = `
    INSERT INTO Usuarios (Usuario, Contrasena, NombreCompleto, CorreoElectronico, FechaRegistro, Rol, Estatus, CodigoVendedor, Direccion, NumeroTelefono, Cedula, Almacen, SupervisadoPor) VALUES (?, ?, ?, ?, DATE_SUB(NOW(), INTERVAL 4 HOUR), ?, 1, ?, ?, ?, ?, ?, ?)`;

  const insertValues = [
    newUserData.usuario, newUserData.contrasena, newUserData.nombreCompleto, newUserData.correoElectronico, newUserData.rol, newUserData.codigoVendedor, newUserData.direccion, newUserData.numeroTelefono, newUserData.cedula, newUserData.almacen, newUserData.supervisadoPor
  ];
