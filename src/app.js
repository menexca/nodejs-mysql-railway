...
app.get('/Productos', async (req, res) => {
  const [rows] = await pool.query(`Select P.CodigoProducto, (case when Marca='01002' then CONCAT(CAST((P.CodigoProducto) AS CHAR),' - ',P.Nombre) else P.Nombre end) as Nombre, P.IVA, P.CantidadxBulto, P.Existencia, P.Existencia02, P.Existencia03, (P.Existencia02 + P.Existencia03) as ExistenciaVenta , (case when P.PedidoVenta is null then 0 else P.PedidoVenta end) as PedidoVenta, PP.PrecioMoneda from Productos P left join ProductosPrecios PP on P.CodigoProducto=PP.CodigoProducto where Existencia>0 and Visible=1 Order by P.Nombre`)
  res.json(rows)
})

// Obtener todos los clientes
app.get('/Clientes', async (req, res) => {
  const [rows] = await pool.query(`SELECT CodigoCliente, Nombre, RazonSocial, Direccion, (case when DiasVisita not in (1,2,3,4,5,6,7) then 8 else DiasVisita end) as DiasVisita, (case when Telefonos='null' then '' else Telefonos end) as Telefonos, IFNULL(LimiteCredito,0) as LimiteCredito, SaldoMonedaTotal, (case when SaldoMonedaVencido>SaldoMonedaTotal then SaldoMonedaTotal else SaldoMonedaVencido end) as SaldoMonedaVencido, date(ultimopago) as ultimopago FROM Clientes WHERE Estatus<>'S' Order by SaldoMonedaTotal desc`)
  res.json(rows)
})
...
app.get('/Clientes/:Vendedor', async (req, res) => {
  ...
  const query = `SELECT CodigoCliente, Nombre, RazonSocial, Direccion, (case when DiasVisita not in (1,2,3,4,5,6,7) then 8 else DiasVisita end) as DiasVisita, (case when Telefonos='null' then '' else Telefonos end) as Telefonos, IFNULL(LimiteCredito,0) as LimiteCredito, SaldoMonedaTotal, (case when SaldoMonedaVencido>SaldoMonedaTotal then SaldoMonedaTotal else SaldoMonedaVencido end) as SaldoMonedaVencido, date(ultimopago) as ultimopago FROM Clientes WHERE Estatus<>'S' and Vendedor = ? Order by SaldoMonedaTotal desc`;
